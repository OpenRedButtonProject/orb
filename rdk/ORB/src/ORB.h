...
public:
  ...
  std::shared_ptr<ApplicationManager> GetApplicationManager() { return _applicationManager; }
  std::shared_ptr<TokenManager> GetTokenManager() { return _tokenManager; }
  std::shared_ptr<BroadcastRequestHandler> GetBroadcastRequestHandler() { return _broadcastRequestHandler; }
  std::shared_ptr<ConfigurationRequestHandler> GetConfigurationRequestHandler() { return _configurationRequestHandler; }
  std::shared_ptr<ManagerRequestHandler> GetManagerRequestHandler() { return _managerRequestHandler; }
  std::shared_ptr<ProgrammeRequestHandler> GetProgrammeRequestHandler() { return _programmeRequestHandler; }
  std::shared_ptr<ParentalControlRequestHandler> GetParentalControlRequestHandler() { return _parentalControlRequestHandler; }
  ORBPlatform *GetORBPlatform() { return _orbPlatform; }

  // orb state setters
  void SetCurrentAppId(uint16_t appId) { _currentAppId = appId; }

  // orb metadata search task pool handling
  void AddMetadataSearchTask(int queryId, std::shared_ptr<MetadataSearchTask> searchTask) { _metadataSearchTasks[queryId] = searchTask; }
  void RemoveMetadataSearchTask(int queryId) { _metadataSearchTasks.erase(queryId); }
  std::shared_ptr<MetadataSearchTask> GetMetadataSearchTask(int queryId) { return _metadataSearchTasks[queryId]; }
