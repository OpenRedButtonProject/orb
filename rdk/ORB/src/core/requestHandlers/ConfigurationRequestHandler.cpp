    ...
    JsonObject& response)
{
    ...
    {
        std::vector<int> dttNetworkIds = ORB::instance(
            nullptr)->GetORBPlatform()->Configuration_GetDttNetworkIds();
    ...
    else if (method == CONFIGURATION_REQUEST_ACCESS_TO_DISTINCTIVE_IDENTIFIER)
    {
        ...
        ORB::instance(
            nullptr)->GetORBPlatform()->Configuration_RequestAccessToDistinctiveIdentifier(origin);
        response.FromString("{}");
    }
    // UnknownMethod
    else
