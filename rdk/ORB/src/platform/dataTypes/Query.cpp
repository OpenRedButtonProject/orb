 ...
 * @param in
 */
void Query::CommonQuery(JsonObject in)
{
    m_comparison = Comparison::CMP_INVALID;
    if (in.HasLabel("queryId"))
    {
        m_queryId = in["queryId"].Number();
    }
    else
    {
        m_queryId = -1;
    }
    if (in.HasLabel("operation"))
    {
        std::string operation = in["operation"].String();
        ...
        JsonArray arguments = in["arguments"].Array();
        if (arguments.Length() > 0)
        {
            JsonObject arg1 = arguments[0].Object();
            m_operator1 = std::make_shared<Query>(arg1);
            if (arguments.Length() > 1)
            {
                JsonObject arg2 = arguments[1].Object();
    ...
    {
        m_field = in["field"].String();
        m_comparison = static_cast<Comparison>(in["comparison"].Number());
        m_value = in["value"].String();
    }
}
