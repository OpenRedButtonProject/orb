...
#define ORB_LOG(msg, ...) do \
    { \
        fprintf(stderr, "ORBPlatformImpl [%s]::[%s]::[%d] ", SIMPLE_FILE_NAME, __FUNCTION__, \
    __LINE__); \
 ...
 * @return true in success, false otherwise
 */
bool ORBPlatformImpl::Dsmcc_SubscribeToStreamEventByName(std::string url, std::string name, int
    listenId)
{
 ...
 * @return true in success, false otherwise
 */
bool ORBPlatformImpl::Dsmcc_SubscribeStreamEventId(std::string name, int componentTag, int eventId,
    int listenId)
{
 ...
 */
std::map<std::string,
         std::vector<ParentalRating> > ORBPlatformImpl::ParentalControl_GetRatingSchemes()
{
 ...
 */
bool ORBPlatformImpl::ParentalControl_IsRatingBlocked(std::string scheme, std::string region, int
    value)
{
    bool blocked = true;
