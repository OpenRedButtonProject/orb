   ...
   json& response)
{
   ...
   {
      std::string preferredAudioLanguage = ORBEngine::GetSharedInstance().GetORBPlatform()->Configuration_GetPreferredAudioLanguage();
   ...
   {
      std::string preferredSubtitleLanguage = ORBEngine::GetSharedInstance().GetORBPlatform()->Configuration_GetPreferredSubtitleLanguage();
   ...
   {
      std::string countryId = ORBEngine::GetSharedInstance().GetORBPlatform()->Configuration_GetCountryId();
      response["result"] = countryId;
   }
   // Configuration.getSubtitlesEnabled
   else if (method == CONFIGURATION_GET_SUBTITLES_ENABLED)
   {
      bool subtitlesEnabled = ORBEngine::GetSharedInstance().GetORBPlatform()->Configuration_GetSubtitlesEnabled();
   ...
   {
      bool audioDescriptionEnabled = ORBEngine::GetSharedInstance().GetORBPlatform()->Configuration_GetAudioDescriptionEnabled();
   ...
   {
      std::string deviceId = ORBEngine::GetSharedInstance().GetORBPlatform()->Configuration_GetDeviceId();
   ...
   else if (method == CONFIGURATION_REQUEST_ACCESS_TO_DISTINCTIVE_IDENTIFIER)
   {
      ...
      std::map<std::string, std::string> appNames = ORBEngine::GetSharedInstance().GetApplicationManager()->GetCurrentAppNames();
      ORBEngine::GetSharedInstance().GetORBPlatform()->Configuration_RequestAccessToDistinctiveIdentifier(origin, appNames);
   }
   // UnknownMethod
   else
   {
...
std::vector<int> ConfigurationRequestHandler::GetDttNetworkIds()
{
   ...
   for (auto channel : channelList)
   {
      ...
      if (idType == Channel::IdType::CHANNEL_ID_DVB_T || idType == Channel::IdType::CHANNEL_ID_DVB_T2)
      {
