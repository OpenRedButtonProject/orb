   ...
   json& response)
{
   ...
   if (method == MANAGER_CREATE_APPLICATION)
   {
      ...
      bool canCreate = ORBEngine::GetSharedInstance().GetApplicationManager()->CreateApplication(appId, url);
   ...
   else if (method == MANAGER_GET_KEY_ICON)
   {
      ...
      std::string keyUri = ORBEngine::GetSharedInstance().GetORBPlatform()->Manager_GetKeyIcon(code);
   ...
   else if (method == MANAGER_SET_KEY_VALUE)
   {
      ...
      response["result"] = ORBEngine::GetSharedInstance().GetApplicationManager()->SetKeySetMask(callingAppId, value);
   }
   ...
   {
      int callingAppId = GetAppIdFromToken(token);
      int value = ORBEngine::GetSharedInstance().GetApplicationManager()->GetKeySetMask(callingAppId);
