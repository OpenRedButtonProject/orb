...
uint32_t ORB::ExecuteWpeBridgeRequest(JsonObject request, JsonObject& response)
{
    ...
    std::string responseAsString = ORBEngine::GetSharedInstance().ExecuteBridgeRequest(
        requestAsString);
...
uint32_t ORB::ApplicationLoadFailed(const ApplicationLoadFailedParamsData& params)
{
    ...
    ORBEngine::GetSharedInstance().NotifyApplicationLoadFailed(params.Url.Value(),
...
{
    SYSLOG(Logging::Notification, (_T("[ORB::ApplicationPageChanged] url=%s"),
                                   url.Value().c_str()));
