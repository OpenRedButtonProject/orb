...
void ORB::RegisterAll()
{
    ...
    JSONRPC::Register<SendKeyEventParamsData, Core::JSON::Boolean>(_T("SendKeyEvent"),
...
uint32_t ORB::SendKeyEvent(const SendKeyEventParamsData& params, Core::JSON::Boolean& response)
{
    ...
    {
        ORB_LOG("Calling the COMRPC SendKeyEvent for %u, %d", params.Keycode.Value(),
            params.Keyaction.Value());

        // define keyaction to use
        uint8_t keyAction =
            params.Keyaction.Value() == SendKeyEventParamsData::KeyactionType::KEY_ACTION_UP ? 0 :
            1;
