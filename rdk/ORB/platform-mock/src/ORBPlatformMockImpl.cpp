 ...
 */
bool ORBPlatformMockImpl::Configuration_RequestAccessToDistinctiveIdentifier(std::string origin, std::map<std::string, std::string> appNames)
{
   ...
   )
{
   ORB_LOG("messageId=%s messageType=%s message=%s drmSystemId=%s blocked=%s",
      messageId.c_str(), messageType.c_str(), message.c_str(), drmSystemId.c_str(), blocked ? "true":"false");
