...
public:
    void Initialize(WKBundleRef bundle, const void *userData = nullptr)
    {
        ...
        {
            TRACE(Trace::Error, (_T("Could not open connection to node %s. Error: %s"),
                                 _comClient->Source().RemoteId(), Core::NumberType<uint32_t>(
                                     result).Text()));
        }
        else
        {
        ...
        g_variant_get((GVariant *) userData, "(&sm&sb)", &uid, &whitelist,
...
private:
    static void automationMilestone(const char *arg1, const char *arg2, const char *arg3)
    {
        ...
        TRACE_GLOBAL(Trace::Information, (_T("TEST TRACE: \"%s\" \"%s\" \"%s\""), arg1, arg2,
                                          arg3));
    }
        ...
        WebKitFrame *frame)
    {
        ...
        JSCValue *function = jsc_value_new_function(jsContext, nullptr,
            reinterpret_cast<GCallback>(automationMilestone),
...
static bool shouldGoToBackForwardListItem(WKBundlePageRef, WKBundleBackForwardListItemRef item,
...
static WKBundlePageLoaderClientV6 s_pageLoaderClient = {
    ...
    [](WKBundlePageRef pageRef, WKBundleFrameRef frame, WKErrorRef error, WKTypeRef *userData, const
       void *clientInfo) {
#if defined(ENABLE_ORB)
    ...
    [](WKBundlePageRef page, WKBundleFrameRef frame, WKBundleScriptWorldRef scriptWorld, const
       void *) {
        bool isMainCtx = (WKBundleFrameGetJavaScriptContext(frame) ==
                          WKBundleFrameGetJavaScriptContextForWorld(frame, scriptWorld));
        if (isMainCtx)
        {
            #if defined(ENABLE_ORB)
...
static WKBundlePageUIClientV4 s_pageUIClient = {
    ...
    [](WKBundlePageRef page, WKConsoleMessageSource source, WKConsoleMessageLevel level, WKStringRef
       message, uint32_t lineNumber,
...
static WKURLRequestRef willSendRequestForFrame(
    WKBundlePageRef page, WKBundleFrameRef, uint64_t, WKURLRequestRef request, WKURLResponseRef,
    const void *)
{
