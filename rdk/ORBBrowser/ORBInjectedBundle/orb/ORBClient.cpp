...
void ORBClient::SubscribeWithJavaScriptEventDispatchRequestedEvent()
{
    ...
    if (m_subscribedEvents["JavaScriptEventDispatchRequestedEvent"] == false)
    {
        error_code = m_remoteObject.Subscribe<JavaScriptEventDispatchRequestedParamsData>(
            TIMEOUT_FOR_ONEWAY_METHODS, _T("javaScriptEventDispatchRequested"),
            &JavaScriptEventDispatchRequested);

        fprintf(stderr,
            "[ORBClient::ORBClient] Subscribe to event 'javaScriptEventDispatchRequested': %s\n",
 ...
 */
void ORBClient::DispatchEvent(std::string type, JsonObject properties, bool broadcastRelated,
