...
public:
    ...
    {
        if ((originIndex.Current().Origin.IsSet() == true) &&
            (originIndex.Current().Domain.IsSet() == true))
        {
            WhiteListedOriginDomainsList::Domains& domains(
                info[originIndex.Current().Origin.Value()]);

            Core::JSON::ArrayType<Core::JSON::String>::Iterator domainIndex(
                originIndex.Current().Domain.Elements());
...
/* static */ unique_ptr<WhiteListedOriginDomainsList> WhiteListedOriginDomainsList::
RequestFromWPEFramework(const char *whitelist)
{
    ...
    WKBundlePostSynchronousMessage(WebKit::Utils::GetBundle(), jsMessageName, messageBody,
        &returnData);
...
void WhiteListedOriginDomainsList::AddWhiteListToWebKit(WKBundleRef bundle)
{
    ...
    while (index != _whiteMap.end())
    {
        ...
        {
            WebKitSecurityOrigin *domain = webkit_security_origin_new_for_uri(
                domainIndex.second.c_str());
