...
public:
   ...
   uint32_t Configure(PluginHost::IShell *service) override
   {
      ...
      bool environmentOverride(ORBBrowser::EnvironmentOverride(_config.EnvironmentOverride.Value()));
      ...
      {
         Core::SystemInfo::SetEnvironment(_T("JSC_useWeakRefs"), _T("true"), !environmentOverride);
      }
...
private:
    ...
    */
   static void resourceFailedCallback(WebKitWebResource *resource, GError *error, gpointer user_data)
   {
      // Get the current app URL from ORB
      std::string currentAppUrl;
      currentAppUrl = ORBWPEWebExtensionHelper::GetSharedInstance().GetORBClient()->GetCurrentAppUrl();
      ...
      if (currentAppUrl == resourceUri)
      {
         ...
         ORBWPEWebExtensionHelper::GetSharedInstance().GetORBClient()->NotifyApplicationLoadFailed(resourceUri, errorDescription);
      }
   }
    ...
    */
   static void resourceLoadStartedCallback(WebKitWebView *webView, WebKitWebResource *resource, WebKitURIRequest *request, gpointer user_data)
   {
      g_signal_connect(resource, "failed", reinterpret_cast<GCallback>(resourceFailedCallback), nullptr);
   }
   ...
   uint32_t Worker() override
   {
      ...
      ORBWPEWebExtensionHelper::GetSharedInstance().SetORBWPEWebExtensionPreferences(preferences, jsonConfigAsString);
      ...
      _view = WEBKIT_WEB_VIEW(g_object_new(WEBKIT_TYPE_WEB_VIEW,
         ...
         "user-content-manager", ORBWPEWebExtensionHelper::GetSharedInstance().CreateWebKitUserContentManager(),
      ...
      g_signal_connect(_view, "resource-load-started", reinterpret_cast<GCallback>(resourceLoadStartedCallback), nullptr);
   ...
   uint32_t Worker() override
   {
      ...
      if (_config.InjectedBundle.Value().empty() == false)
      {
         ...
         {
            bundlePath.reset( g_build_filename("/usr/share/WPEFramework/ORBBrowser/", _config.InjectedBundle.Value().c_str(), nullptr));
         }
         ...
         fprintf(stderr, "WPE_INJECTED_BUNDLE_PATH=%s\n", WKStringToString(injectedBundlePathString).c_str());
         setenv("WPE_INJECTED_BUNDLE_PATH", WKStringToString(injectedBundlePathString).c_str(), 1);
   ...
   WKTypeRef messageBodyObj, WKTypeRef *returnData, const void *clientInfo)
{
   ...
   else if (name == Tags::DispatchEvent || name == Tags::Action)
   {
      ...
      fprintf(stderr, "[WebKitImplementation::onDidReceiveSynchronousMessageFromInjectedBundle] %s\n", info.c_str());
