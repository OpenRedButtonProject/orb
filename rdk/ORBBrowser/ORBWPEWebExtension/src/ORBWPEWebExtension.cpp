...
static gboolean EvaluateJavaScript(gpointer javascript)
{
   ...
   g_autoptr(JSCValue) result = jsc_context_evaluate(s_jsContext, kScript.c_str(), kScript.length());
...
static void ExposeToken(JSCContext *jsContext, const char *uri)
{
   ...
   g_autoptr(JSCValue) result = jsc_context_evaluate(jsContext, javascript.c_str(), javascript.length());
   g_clear_object(&result);
}
 ...
 */
static void OnWindowObjectCleared(WebKitScriptWorld *world, WebKitWebPage *page, WebKitFrame *frame, gpointer userData)
{
   ORB_LOG_NO_ARGS();
...
static void OnPageUriChanged(WebKitWebPage *webPage, GParamSpec *pspec, WebKitWebExtension *extension)
{
   ...
   ORB_LOG("Whitelisting orb URLs for origin: %s", webkit_security_origin_to_string(originReverse));
   webkit_web_extension_add_origin_access_whitelist_entry(s_orbWpeWebExtension, originReverse, "http", "", true);
   webkit_web_extension_add_origin_access_whitelist_entry(s_orbWpeWebExtension, originReverse, "https", "", true);
   webkit_web_extension_add_origin_access_whitelist_entry(s_orbWpeWebExtension, originReverse, "dash", "", true);
 ...
 */
extern "C"
void webkit_web_extension_initialize_with_user_data(WebKitWebExtension *extension, const GVariant *userData)
{
   ORB_LOG_NO_ARGS();

   g_signal_connect(webkit_script_world_get_default(), "window-object-cleared", G_CALLBACK(OnWindowObjectCleared), (gpointer)userData);
