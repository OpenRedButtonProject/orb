...
void ORBDVBURILoader::Finish()
{
   ...
   else
   {
      ...
      GError *error = g_error_new(g_quark_from_string(failedUri.c_str()), 0, errorDescription.c_str());
      ...
      {
         ORBWPEWebExtensionHelper::GetSharedInstance().GetORBClient()->NotifyApplicationLoadFailed(failedUri, errorDescription);
      }
   }
