...
DnsSrvResolver::DnsSrvResolver(const std::string& dnsServer, int timeoutMs)
    : m_dnsServer(dnsServer)
    , m_timeoutMs(timeoutMs)
{
}

std::vector<uint8_t> DnsSrvResolver::BuildDnsQuery(const std::string& name,
...
std::string DnsSrvResolver::ParseDomainName(const uint8_t* response,
                                             size_t responseLen,
                                             size_t& offset)
{
    std::string name;
...
std::vector<SrvRecord> DnsSrvResolver::ParseDnsResponse(const uint8_t* response,
                                                         size_t length)
{
...
std::vector<SrvRecord> DnsSrvResolver::Query(const std::string& serviceName)
{
    std::vector<SrvRecord> records;

    LOG(INFO) << "DNS SRV query for: " << serviceName;
    ...
    if (inet_pton(AF_INET, m_dnsServer.c_str(), &serverAddr.sin_addr) != 1) {
        LOG(ERROR) << "Invalid DNS server address: " << m_dnsServer;
        close(sock);
        return records;
    }
