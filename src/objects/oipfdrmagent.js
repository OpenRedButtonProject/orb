...
hbbtv.objects.OipfDrmAgent = (function() {
   ...
   prototype.sendDRMMessage = function(msgType, msg, DRMSystemID) {
      ...
      if (!DrmMimeTypes.includes(msgType)) {
         Promise.resolve().then(() => dispatchDRMMessageResult.call(this, msgId, null, Result.MSG_UNKNOWN_MIME_TYPE));
      }
      else if (p.drmSystemIdStatusMap.has(DRMSystemID)) {
         hbbtv.bridge.drm.sendDRMMessage(msgId, msgType, msg, DRMSystemID);
      } else {
         Promise.resolve().then(() => dispatchDRMMessageResult.call(this, msgId, null, Result.MSG_UNKNOWN_DRM_SYSTEM));
      }
      return msgId;
   }
   ...
   function initialise() {
      ...
      sysIds.forEach(element => p.drmSystemIdStatusMap.set(element.DRMSystemID, element.status));
