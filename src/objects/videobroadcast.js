...
hbbtv.objects.VideoBroadcast = (function() {
    ...
    function cycleChannel(delta) {
        ...
        if (p.playState === this.PLAY_STATE_UNREALIZED || p.channelConfig.channelList.length < 2) {
            dispatchChannelChangeErrorEvent.call(
                this,
                p.currentChannelData,
                this.CHANNEL_STATUS_CANNOT_BE_CHANGED
            );
            return;
        }
        let i;
    ...
    prototype.unselectComponent = function(component) {
        ...
        if (!isNaN(component)) {
            ...
            if (
                (component.type === 1 &&
                    hbbtv.bridge.configuration.getPreferredAudioLanguage() !=
                    null) /* Return to the default preferred audio language. */ ||
                (component.type === 2 &&
                    hbbtv.bridge.configuration.getPreferredSubtitleLanguage() != null)
            ) {
                ...
                if (defaultComponent) {
                    // && !defaultComponent.active) {//Note: we can't rely on active to avoid repetition as it won't be updated until events with the result are received
                    hbbtv.bridge.broadcast.selectComponent(
                        defaultComponent.type,
                        defaultComponent.pid,
                        ''
                    );
                }
                return;
