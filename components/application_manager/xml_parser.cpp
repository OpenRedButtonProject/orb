...
static void XmlParseAppName(xmlNodePtr node, Ait::S_APP_NAME_DESC *app_name)
{
   ...
   if (index < app_name->num_langs)
   {
      ...
      if (dptr != nullptr)
      {
         app_name->names[index].name = std::string(reinterpret_cast<char *>(dptr), xmlStrlen(dptr));
         NODE_CONTENT_RELEASE();
      }
   }
}
            ...
            while (node != nullptr)
            {
               if (node->type == XML_ELEMENT_NODE)
               {
                  ...
                  if (xmlStrEqual(cptr, (const xmlChar *)"URLBase"))
                  {
                     ...
                     if (dptr)
                     {
                        trns_ptr->url.base_url = std::string(reinterpret_cast<char *>(dptr), xmlStrlen(dptr));
                        NODE_CONTENT_RELEASE();
                     }
                  }
                  else if (xmlStrEqual(cptr, (const xmlChar *)"URLExtension"))
                  {
                     ...
                     if (dptr)
                     {
                        trns_ptr->url.extension_urls.emplace_back(reinterpret_cast<char *>(dptr),
                           xmlStrlen(dptr));
                        NODE_CONTENT_RELEASE();
                     }
                  }
                ...
                xmlStrEqual(dnode->name, (const xmlChar *)"ApplicationList"))
            {
               ...
               while (lnode != nullptr)
               {
                      ...
                      xmlStrEqual(lnode->name, (const xmlChar *)"Application"))
                  {
                     ...
                     XmlAllocApplication(lnode->xmlChildrenNode, &ait_table->app_array[index]);
                     XmlParseApplication(lnode->xmlChildrenNode, &ait_table->app_array[index]);
                     index++;
                  }
                  lnode = lnode->next;
               }
 ...
 * @param strptr
 */
static void XmlParseDsmObjects(xmlNodePtr dsm_node, XmlParser::S_XML_SE_OBJ *se_obj, XmlParser::S_XML_SEVENT *sevent, uint8_t *strptr)
{
   uint16_t ecnt;
   xmlChar *dptr;
   xmlNodePtr se_node;
...
XmlParser::S_XML_DSMCC * XmlParser::ParseDsmcc(uint8_t *content, uint32_t length)
{
   ...
   else
   {
      ...
      else if (node->type != XML_ELEMENT_NODE || !xmlStrEqual(node->name, (const xmlChar *)"dsmcc"))
      {
