...
public:
   ...
   static TimelineSource* Make(const std::string &timelineSelector, ClockBase *wallclock, double tickRate)
   {
      CorrelatedClock *correlatedClock = new CorrelatedClock(wallclock, Correlation(wallclock->getTicks(), 0), tickRate);
      SimpleClockTimelineSource *tls = new SimpleClockTimelineSource(timelineSelector, wallclock, correlatedClock);
...
std::unordered_map<TimelineSource *, CorrelatedClock *> SimpleClockTimelineSourceFactory::correlatedClocks_ = {};

////////////////////////////////////////////////////////
////////////////// MediaSynchroniser ///////////////////
////////////////////////////////////////////////////////

MediaSynchroniser::MediaSynchroniser(const int &id, std::shared_ptr<MediaSyncCallback> mediaSyncCallback, const int &ciiPort, const int &wcPort, const int &tsPort) :
   m_id(id),
...
void MediaSynchroniser::initialise(const bool &isMasterBroadcast, std::string dvbUri, bool permanentError, bool presenting)
{
...
void MediaSynchroniser::updateBroadcastContentStatus(std::string dvbUri, bool permanentError, bool presenting)
{
   std::lock_guard<std::recursive_mutex> lockGuard(m_mutex);
   if (presenting)
   {
      for (auto &it : m_timelines)
      {
         if (it.second.temiFilterId == -1 && it.second.temiComponentTag != -1 && it.second.temiTimelineId != -1)
         {
            it.second.temiFilterId = m_mediaSyncCallback->startTEMITimelineMonitoring(it.second.temiComponentTag, it.second.temiTimelineId);
         }
      }
   }
...
{
   LOG(LOG_INFO, "MediaSynchroniser::updateCssCiiProperties(%s, %s, %s, %s)\n", contentId.c_str(), presentationStatus.c_str(), contentIdStatus.c_str(), mrsUrl.c_str());
   std::lock_guard<std::recursive_mutex> lockGuard(m_mutex);
...
bool MediaSynchroniser::enableInterDeviceSync(const std::string &ipAddr)
{
   ...
   if (m_runningServices <= 0 && m_initialised)
   {
      ...
      if (m_ciiService > -1 && m_wcService > -1 && m_tsService > -1)
      {
         ...
         ret = true;
         TimelineSyncService *ts = ServiceManager::GetInstance().FindService<TimelineSyncService>(m_tsService);
...
int MediaSynchroniser::nrOfSlaves()
{
   std::lock_guard<std::recursive_mutex> lockGuard(m_mutex);
   int ret = -1;
   ContentIdentificationService *cii = ServiceManager::GetInstance().FindService<ContentIdentificationService>(m_ciiService);
   if (cii != nullptr)
   {
      ret = cii->nrOfClients();
   }
...
bool MediaSynchroniser::setContentTimeAndSpeed(std::string timelineSelector, const uint64_t &contentTime, const double &speed)
{
   std::lock_guard<std::recursive_mutex> lockGuard(m_mutex);
...
bool MediaSynchroniser::setTimelineAvailability(const std::string &timelineSelector, const bool &isAvailable, const u_int64_t currentTime, const double speed)
{
   LOG(LOG_DEBUG, "MediaSynchroniser::setTimelineAvailability(%s, %d, %llu, %f)", timelineSelector.c_str(), isAvailable, currentTime, speed);
   bool result = false;
   TimelineSource *tls = _getTimelineSource(timelineSelector);
   if (tls != nullptr)
   {
...
bool MediaSynchroniser::setTEMITimelineAvailability(const int &filterId, const bool &isAvailable, const u_int64_t &currentTime, const u_int64_t& timescale, const double &speed)
{
   LOG(LOG_DEBUG, "MediaSynchroniser::setTEMITimelineAvailability(%d, %d, %llu, %llu, %f)", filterId, isAvailable, currentTime, timescale, speed);
   std::lock_guard<std::recursive_mutex> lockGuard(m_mutex);
   bool result = false;
...
bool MediaSynchroniser::startTimelineMonitoring(const std::string &timelineSelector, const bool &isMaster)
{
   bool result = false;
   std::lock_guard<std::recursive_mutex> lockGuard(m_mutex);
   if (m_initialised)
      ...
      if (timelineWrapper.numWatchers <= 0)
      {
         if (timelineWrapper.pendingWatchers <= 0)
         {
            if (ParseTimelineSelector(timelineSelector, timelineWrapper))
            {
               ...
               if (timelineWrapper.temiComponentTag != -1 && timelineWrapper.temiTimelineId != -1)
               {
                  int filterId = m_mediaSyncCallback->startTEMITimelineMonitoring(timelineWrapper.temiComponentTag, timelineWrapper.temiTimelineId);
                  if (filterId >= 0)
                  {
                  ...
                  {
                     LOG(LOG_ERROR, "Failed to start timeline monitoring for temi timeline '%s'.\n", timelineSelector.c_str());
                  }
               }
               else
               {
         ...
         else
         {
            ...
            LOG(LOG_DEBUG, "Incremented pending timeline monitoring counter for timeline selector '%s'. Current count is now %d.\n", timelineSelector.c_str(), timelineWrapper.pendingWatchers);
         }
      }
      else
      {
         timelineWrapper.numWatchers++;
         LOG(LOG_DEBUG, "Incremented timeline monitoring counter for timeline selector '%s'. Current count is now %d.\n", timelineSelector.c_str(), timelineWrapper.numWatchers);
      }
   }
   else
   {
      LOG(LOG_ERROR, "Cannot start timeline monitoring on a media synchroniser that is not initialised.\n");
   }
...
bool MediaSynchroniser::stopTimelineMonitoring(std::string timelineSelector, bool forceStop)
{
   ...
   {
      timelineWrapper.pendingWatchers--;
      LOG(LOG_DEBUG, "Decremented pending timeline monitoring counter for timelineSelector '%s'. Current count is now %d.\n", timelineSelector.c_str(), timelineWrapper.pendingWatchers);
   }
   else if (timelineWrapper.numWatchers > 1)
   {
      timelineWrapper.numWatchers--;
      LOG(LOG_DEBUG, "id=%d. Decremented reference counter for timelineSelector '%s'. Current count is now %d.\n", m_id, timelineSelector.c_str(), timelineWrapper.numWatchers);
   }
   else
   {
            ...
            if (tls == nullptr)
            {
               ...
               if (m_masterTimeline == timelineSelector)
               {
                  ...
                  if (clockBase != nullptr)
                  {
                     for (auto &it : m_timelineSources)
                     {
                        CorrelatedClock *clock = SimpleClockTimelineSourceFactory::GetCorrelatedClock(it.second);
                        if (clock != nullptr)
                        {
...
void MediaSynchroniser::removeTimeline(const std::string &timelineSelector)
{
   ...
   {
      LOG(LOG_DEBUG, "A timeline source for timeline selector '%s' was not found. Cleaning up counters...\n", timelineSelector.c_str());
   }
   m_timelines.erase(timelineSelector);
}
...
MediaSynchroniserManager::MediaSynchroniserManager(std::shared_ptr<MediaSyncCallback> mediaSyncCallback, const int &ciiPort, const int &wcPort, const int &tsPort) :
...
int MediaSynchroniserManager::createMediaSynchroniser()
{
   std::lock_guard<std::recursive_mutex> lockGuard(m_mutex);
   int id = m_idCounter++;
   m_mediaSyncs[id] = new MediaSynchroniser(id, m_mediaSyncCallback, m_ciiPort, m_wcPort, m_tsPort);
   return id;
}
...
void MediaSynchroniserManager::updateDvbInfo(const std::string &dvbUri, bool permanentError, bool presenting)
{
   LOG(LOG_DEBUG, "MediaSynchroniserManager::updateDvbInfo(%s,%d,%d).\n", dvbUri.c_str(), permanentError, presenting);
   m_dvbUri = dvbUri;
...
bool MediaSynchroniser::ParseTimelineSelector(const std::string &timelineSelector, TimelineWrapper &timelineWrapper)
{
   Json::Value &timeline = timelineWrapper.timeline;
   ...
   if (pos != std::string::npos)
   {
      ...
      if (selectorParts.size() > 0)
      {
         ...
         {
            result = CSSUtilities::unpack("{\"timelineSelector\":\"" + timelineSelector + "\",\"timelineProperties\":{\"unitsPerTick\":1,\"unitsPerSecond\":1000}}", timeline);
         }
         else if (selectorParts[0] == "pts")
         {
            result = CSSUtilities::unpack("{\"timelineSelector\":\"" + timelineSelector + "\",\"timelineProperties\":{\"unitsPerTick\":1,\"unitsPerSecond\":90000}}", timeline);
         }
         else if (selectorParts[0] == "mpd")
         {
            ...
            {
               timeline["timelineProperties"]["unitsPerSecond"] = static_cast<Json::UInt64>(std::stoull(selectorParts[3]));
            }
         }
         else if (selectorParts[0] == "temi" && selectorParts.size() >= 3)
         {
            result = CSSUtilities::unpack("{\"timelineSelector\":\"" + timelineSelector + "\"},\"componentTag\":" + selectorParts[1] + ",\"timelineId\":" + selectorParts[2] + "}", timeline);
