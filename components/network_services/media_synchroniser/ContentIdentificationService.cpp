   ...
   const Json::Value &value)
{
   ...
   if (!key.empty())
   {
      ...
      if (std::find(std::begin(CSSUtilities::CIIMessageProperties::keys), std::end(CSSUtilities::CIIMessageProperties::keys), key) !=
      ...
      {
         LOG(LOG_ERROR, "Key property name does not match with CII message protocol property names\n");
      }
   }
   else
   {
...
ContentIdentificationService::ContentIdentificationService(int port, ContentIdentificationProperties *props) :
...
void ContentIdentificationService::OnMessageReceived(WebSocketService::WebSocketConnection *connection,
   const std::string &text)
{
   LOG(LOG_INFO, "Received unexpected message on connection %s: %s\n", connection->Uri().c_str(), text.c_str());
}
...
std::string ContentIdentificationService::pack(const Json::Value &currentMessage, bool onlydiff, bool alwaysSendTimelines)
{
