...
SetupTSData SetupTSData::unpack(const std::string &msg)
{
   ...
   if (CSSUtilities::unpack(msg, root))
   {
      if (!root["timelineSelector"].isNull())
      {
         ...
         return SetupTSData(root["contentIdStem"].asString(), root["timelineSelector"].asString(), {});
      }
   }
...
AptEptLpt AptEptLpt::unpack(const std::string &msg)
{
   ...
   try
   {
      if (CSSUtilities::unpack(msg, root))
      {
         if (root["actual"]["wallClockTime"].isUInt64() && root["actual"]["contentTime"].isUInt64())
         {
            if (!root["actual"]["wallClockTime"].isNull() && !root["actual"]["contentTime"].isNull())
            {
               actualTStamp.setTimeStamp(root["actual"]["contentTime"].asUInt64(),
                  root["actual"]["wallClockTime"].asString());
            }

            if (!root["earliest"]["wallClockTime"].isNull() && !root["earliest"]["contentTime"].isNull())
            {
               earliestTStamp.setTimeStamp(root["earliest"]["contentTime"].asUInt64(),
                  root["earliest"]["wallClockTime"].asString());
            }

            if (!root["latest"]["wallClockTime"].isNull() && !root["latest"]["contentTime"].isNull())
            {
...
Nullable<ControlTimestamp> &SimpleTimeLineSource::getControlTimestamp(const std::string &timelineSelector)
{
   return m_ctStamp;
}
...
Nullable<ControlTimestamp> &SimpleClockTimelineSource::getControlTimestamp(const std::string &timelineSelector)
{
   if (m_changed)
   {
      m_changed = false;
      ...
      {
         m_latestCt = ControlTimestamp(TimeStamp(m_clock->getTicks(), std::to_string(m_wallClock->getTicks())),
...
//TS SERVICE
TimelineSyncService::TimelineSyncService(int port, ClockBase *wallClock, MediaSynchroniser *mediaSync,
                                         ContentIdentificationService *cii, std::string contentIdOverride) : WebSocketService("lws-ts", port, false, ""),
   m_wallclock{wallClock}, m_mediaSync(mediaSync), m_ciiService{cii}, m_contentIdOverride{contentIdOverride}
{
}
   ...
   if (!m_connectionSetupData[connection].isEmpty())
   {
      ...
      if (ciMatchesStem(getContentId(), m_connectionSetupData[connection].getContentIdStem()))
      {
         for (auto &src : m_timelineSources)
         {
            ...
            {
               ct = src.first->getControlTimestamp(m_connectionSetupData[connection].getTimelineSelector());
            }
         }
      }
      else
      ...
      if (!ct.isNull() && isControlTimestampChanged(m_connectionPreviousControlTimestamp[connection], ct))
      {
...
bool TimelineSyncService::ciMatchesStem(const std::string &contentId, const std::string &contentIdStem)
{
   return(!contentId.empty() && ((contentId.rfind(contentIdStem, 0) == 0) || contentIdStem.empty()));
}
...
void TimelineSyncService::configureConnectionWithSetupData(WebSocketService::WebSocketConnection *connection, const std::string &text)
{
   m_connectionSetupData[connection] = SetupTSData::unpack(text);
   if (m_connectionSetupData[connection].isEmpty())
   {
      LOG(LOG_ERROR, "Unexpected setup data (%s) from %s\n", text.c_str(), connection->Uri().c_str());
   }
   else
   {
