...
class TimeStamp {
public:
   explicit TimeStamp(const Nullable<unsigned long long> &contentTime = Nullable<unsigned long long>(), std::string wallclockTime = "0");
...
public:
   explicit ControlTimestamp(const TimeStamp &tstamp = TimeStamp(), const Nullable<float> &timelineSpeedMultiplier = Nullable<float>())
...
public:
   explicit AptEptLpt(const TimeStamp &earliest = TimeStamp(0, "-inf"),
                      const TimeStamp &latest = TimeStamp(0, "+inf"), const Nullable<TimeStamp> &actual = Nullable<TimeStamp>()) :
...
public:
   ...
   virtual Nullable<ControlTimestamp> &getControlTimestamp(const std::string &timelineSelector) = 0;
...
public:
   ...
   SimpleTimeLineSource(const std::string &timelineSelector, Nullable<ControlTimestamp> &ctStamp) : TimelineSource{
                                                                                                                   timelineSelector}, m_ctStamp{ctStamp}
   {
   };
...
public:
   TimelineSyncService(int port, ClockBase *wallClock, MediaSynchroniser *mediaSync, ContentIdentificationService *cii, std::string contentIdOverride = "");
...
//        };
   ...
   std::unordered_map<WebSocketConnection *, Nullable<ControlTimestamp> > m_connectionPreviousControlTimestamp;
   ...
   bool isControlTimestampChanged(const Nullable<ControlTimestamp> & prev, const Nullable<ControlTimestamp> &latest) const;
   void configureConnectionWithSetupData(WebSocketService::WebSocketConnection *connection, const std::string &text);
};
}
