...
u_int64_t CorrelatedClock::getTicks() const
{
   return mCorrelation.getChildTicks() +
          (mParent->getTicks() - mCorrelation.getParentTicks()) / mParent->getTickRate() * mFreq * getSpeed();;
}
...
double CorrelatedClock::isChangeSignificant(Correlation &newCorrelation, double newSpeed, double thresholdSecs)
{
...
{
   double parentTicks = toParentTicks(ticks);
   double deltaSecs = std::abs(parentTicks - mCorrelation.getParentTicks()) / mParent->getTickRate();
...
void CorrelatedClock::rebaseCorrelationAtTicks(unsigned long tickValue)
{
   if (tickValue > 0)
   {
      ...
      double initError = mCorrelation.getInitialError() + deltaSecs * mCorrelation.getErrorGrowthRate();
