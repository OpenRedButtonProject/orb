...
u_int64_t ClockBase::toOtherClockTicks(ClockBase &otherClock, u_int64_t ticks)
{
   ...
   set_intersection(ancestors.begin(), ancestors.end(), otherAncestors.begin(), otherAncestors.end(),
   ...
   else
   {
      for (auto elem : intersect)
      {
         ...
         otherAncestors.erase(std::remove(otherAncestors.begin(), otherAncestors.end(), elem), otherAncestors.end());
      }
