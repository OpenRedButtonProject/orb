...
WebSocketService::WebSocketService(const std::string &protocol_name, int port, bool use_ssl,
   ...
   retry_{.secs_since_valid_ping = SECS_SINCE_VALID_PING, .secs_since_valid_hangup = SECS_SINCE_VALID_HANGUP},
   ...
   std::vector<uint8_t> &&data, bool is_first, bool is_final, bool is_binary)
{
   ...
   if (!is_binary)
   {
      if (is_first)
      {
         connection->text_buffer_ = std::string(reinterpret_cast<char *>(data.data()), data.size());
      }
      else
      {
         connection->text_buffer_ += std::string(reinterpret_cast<char *>(data.data()), data.size());
