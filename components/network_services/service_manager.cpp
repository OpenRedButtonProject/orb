...
int ServiceManager::StartWallClockService(std::unique_ptr<ServiceCallback> callback, const int &port, ::SysClock *sysClock)
{
   mutex_.lock();
   int id = NewServiceId();
   if (id > -1)
...
int ServiceManager::StartContentIdentificationService(std::unique_ptr<ServiceCallback> callback, const int &port, ContentIdentificationProperties *props)
{
   mutex_.lock();
   int id = NewServiceId();
   if (id > -1)
   {
      std::unique_ptr<ContentIdentificationService> service = std::unique_ptr<ContentIdentificationService>(
...
int ServiceManager::StartTimelineSyncService(std::unique_ptr<ServiceCallback> callback, const int &port, ClockBase *wallclock, MediaSynchroniser *ms, int ciiService)
{
   mutex_.lock();
...
int ServiceManager::StartApp2AppService(std::unique_ptr<ServiceCallback> callback, int localApp2AppPort, int remoteApp2AppPort)
{
   mutex_.lock();
   int id = NewServiceId();
   if (id > -1)
