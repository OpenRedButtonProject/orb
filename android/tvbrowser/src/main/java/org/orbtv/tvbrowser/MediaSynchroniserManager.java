...
public class MediaSynchroniserManager {
    private static final String TAG = "Orb/MediaSynchroniserManager";
    private SessionCallback mSessionCallback;
    private final Object mLock = new Object();

    private long mNativeManagerPointerField; // Reserved for native library

    MediaSynchroniserManager(TvBrowser.Configuration config) {
    ...
    public void updateBroadcastContentStatus(int onetId, int transId, int servId, int statusCode,
                                             boolean permanentError, List<TvBrowserTypes.Programme> programmes) {
        ...
        if (programmes != null && programmes.size() > 0) {
            TvBrowserTypes.Programme programme = programmes.get(0);
        ...
        switch (statusCode) {
            case TvBrowserTypes.CHANNEL_STATUS_PRESENTING:
            case TvBrowserTypes.CHANNEL_STATUS_CONNECTING:
                jniUpdateDvbInfo(dvbUri,
                        pError,
                        statusCode == TvBrowserTypes.CHANNEL_STATUS_PRESENTING);
                break;
            default:
                break;
        }
    ...
    private void jniCbDispatchTimelineAvailableEvent(String timelineSelector, long unitsPerSecond) {
        synchronized (mLock) {
            if (mSessionCallback != null) {
                try {
                    TvBrowserTypes.Timeline timeline = new TvBrowserTypes.Timeline(timelineSelector);
